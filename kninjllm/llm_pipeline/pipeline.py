...
from kninjllm import llm_tracing

from kninjllm.llm_common.component import Component, InputSocket, OutputSocket, component
from kninjllm.llm_common.errors import (
...
from kninjllm.llm_common.serialization import component_from_dict, component_to_dict
from kninjllm.llm_utils.type_utils import _type_name, _types_are_compatible
from kninjllm.llm_marshal import Marshaller, YamlMarshaller
from kninjllm.llm_telemetry import pipeline_running
from kninjllm.llm_utils.jupyter_utils import is_in_jupyter

from kninjllm.llm_pipeline.descriptions import find_pipeline_inputs, find_pipeline_outputs
from kninjllm.llm_pipeline.draw import _to_mermaid_image
from kninjllm.llm_pipeline.template import PipelineTemplate, PredefinedPipeline
...
class Pipeline:
    ...
    def __init__(
        ...
        max_loops_allowed: int = 999999,
        debug_path: Union[Path, str] = Path(".kninjllm_debug/"),
    ):
        """
    ...
    def from_dict(cls: Type[T], data: Dict[str, Any], **kwargs) -> T:
        ...
        debug_path = Path(data.get("debug_path", ".kninjllm_debug/"))
    ...
    def add_component(self, name: str, instance: Component) -> None:
        ...
        if getattr(instance, "__kninjllm_added_to_pipeline__", None):
            msg = (
        ...
        setattr(instance, "__kninjllm_added_to_pipeline__", self)
        ...
        self.graph.add_node(
            name,
            instance=instance,
            input_sockets=instance.__kninjllm_input__._sockets_dict,  # type: ignore[attr-defined]
            output_sockets=instance.__kninjllm_output__._sockets_dict,  # type: ignore[attr-defined]
            visits=0,
        )
    ...
    def get_component(self, name: str) -> Component:
        ...
        try:
            ...
            print(f"Component named {name} not found in the pipeline.")
            return None
    ...
    def _validate_input(self, data: Dict[str, Any]):
        ...
        * Each Component doesn't receive inputs that are already sent by another Component
        ...
        for component_name, component_inputs in data.items():
            ...
            for socket_name, socket in instance.__kninjllm_input__._sockets_dict.items():
            ...
            for input_name in component_inputs.keys():
                if input_name not in instance.__kninjllm_input__._sockets_dict:
    ...
    def run(  # noqa: C901, PLR0912, PLR0915 pylint: disable=too-many-branches
        ...
        # defined in `kninjllm/pipeline.py`.
        ...
        for component_name, component_inputs in data.items():
            ...
            for component_input, input_value in component_inputs.items():
                ...
                if instance.__kninjllm_input__._sockets_dict[component_input].is_variadic:
        ...
        for node_name in self.graph.nodes:
            ...
            if len(component.__kninjllm_input__._sockets_dict) == 0:
                # Component has no input, can run right away
                to_run.append((node_name, component))
                continue

            for socket in component.__kninjllm_input__._sockets_dict.values():
        ...
        with llm_tracing.tracer.trace(
            "kninjllm.pipeline.run",
            tags={
                "kninjllm.pipeline.debug": debug,
                "kninjllm.pipeline.metadata": self.metadata,
                "kninjllm.pipeline.max_loops_allowed": self.max_loops_allowed,
            },
        ):
            ...
            while len(to_run) > 0:
                ...
                if any(socket.is_variadic for socket in comp.__kninjllm_input__._sockets_dict.values()) and not getattr(  # type: ignore
                    comp, "is_greedy", False
                ):
                    there_are_non_variadics = False
                    for _, other_comp in to_run:
                        if not any(socket.is_variadic for socket in other_comp.__kninjllm_input__._sockets_dict.values()):  # type: ignore
                ...
                if name in last_inputs and len(comp.__kninjllm_input__._sockets_dict) == len(last_inputs[name]):  # type: ignore
                    ...
                    with llm_tracing.tracer.trace(
                        "kninjllm.component.run",
                        tags={
                            "kninjllm.component.name": name,
                            "kninjllm.component.type": comp.__class__.__name__,
                            "kninjllm.component.input_types": {
                                k: type(v).__name__ for k, v in last_inputs[name].items()
                            },
                            "kninjllm.component.input_spec": {
                                key: {
                                    "type": value.type.__name__ if isinstance(value.type, type) else str(value.type),
                                    "senders": value.senders,
                                }
                                for key, value in comp.__kninjllm_input__._sockets_dict.items()  # type: ignore
                            },
                            "kninjllm.component.output_spec": {
                                key: {
                                    "type": value.type.__name__ if isinstance(value.type, type) else str(value.type),
                                    "senders": value.receivers,
                                }
                                for key, value in comp.__kninjllm_output__._sockets_dict.items()  # type: ignore
                            },
                        },
                    ) as span:
                        span.set_content_tag("kninjllm.component.input", last_inputs[name])
                        ...
                        span.set_tags(tags={"kninjllm.component.visits": self.graph.nodes[name]["visits"]})
                        span.set_content_tag("kninjllm.component.output", res)
                    ...
                    for sender_component_name, receiver_component_name, edge_data in self.graph.edges(data=True):
                        ...
                        is_greedy = pair[1].__kninjllm_is_greedy__
                ...
                if len(to_run) == 0 and len(waiting_for_input) > 0:
                    ...
                    if (
                        ...
                        for name, comp in waiting_for_input:
                            is_variadic = any(socket.is_variadic for socket in comp.__kninjllm_input__._sockets_dict.values())  # type: ignore
                            if is_variadic and not comp.__kninjllm_is_greedy__:  # type: ignore[attr-defined]
                                break
                        else:
                    ...
                    for name, comp in waiting_for_input:
                        ...
                        if is_variadic and not comp.__kninjllm_is_greedy__:  # type: ignore[attr-defined]
                            ...
                            for other_name, other_comp in waiting_for_input:
                                ...
                                there_are_only_lazy_variadics &= (
                                    any(
                                        socket.is_variadic for socket in other_comp.__kninjllm_input__._sockets_dict.values()  # type: ignore
                                    )
                                    and not other_comp.__kninjllm_is_greedy__  # type: ignore[attr-defined]
                                )
                        ...
                        for input_socket in comp.__kninjllm_input__._sockets_dict.values():  # type: ignore
    ...
    def from_template(
        ...
        # template, we add it to the error kninjllm for debugging
        try:
