...
def call_model_beam_batch(prompt, model, max_new_tokens=15, retriever=None, query=None, max_depth=5, rel_tokens=None,
...
def main(args):
    ...
    def generate(prompt, retriever, max_new_tokens):
        processed_prompt = PROMPT_DICT["prompt_no_input"].format_map(
            {"instruction": prompt})
        return call_model_beam_batch(processed_prompt, model=model, max_new_tokens=max_new_tokens, retriever=retriever, query=prompt,
    ...
    if args.task is not None and args.task == "factscore":
        ...
        for idx, item in enumerate(input_data):
            ...
            result, intermediate = generate(prompt, retriever, args.max_new_tokens,)
        ...
        for instance_idx, item in enumerate(input_data):
            ...
            final_pred, intermediate = generate(
                prompt, retriever, args.max_new_tokens,)
            final_output = ""
            docs = []
            prev_gen = []
