...
def verbalizer(input_data):
    ...
    # do_predict
    args.data_dir="./data/"
    args.test_name="test_table_name" # .source
    args.output_dir="./temp_data_verbalizer_predict"
    
    with open(args.data_dir+args.test_name,'w',encoding='utf-8') as f:
        f.writelines(line + '\n' for line in input_data)
    shutil.copy(args.data_dir+args.test_name, args.data_dir+args.test_name+".source")
    shutil.copy(args.data_dir+args.test_name, args.data_dir+args.test_name+".target")
            
    # with open(args.data_dir+args.test_name+'.source','w',encoding='utf-8') as f:
    #     f.writelines(line + '\n' for line in input_data)    
    # with open(args.data_dir+args.test_name+'.target','w',encoding='utf-8') as f:
    #     f.writelines(line + '\n' for line in input_data)
    
    args.task="graph2text"
    args.model_name_or_path=RootConfig.T5_model_path
    # args.model_name_or_path=RootConfig.verbalizer_model_path
    # args.eval_batch_size=16
    args.eval_batch_size=160
    args.gpus=1
    args.checkpoint=RootConfig.verbalizer_model_path
    args.max_source_length=384
    args.max_target_length=384
    args.val_max_target_length=384
    args.test_max_target_length=384
    args.eval_max_gen_length=384
    args.do_predict=True
    args.eval_beams=1
    args.progress_bar_refresh_rate=1
    args.num_returned_sequence=1
    args.num_shards=1
    args.shard_id=0
    args.gpu_id=0
    args.total_num_gpus=1

    main(args)
    
    verbalizer_outputs=args.output_dir+"/val_outputs/test_table_name_predictions_rank0.txt"
    # selected_sentences = post_processor(verbalizer_inputs=args.data_dir+args.test_name,verbalizer_outputs=verbalizer_outputs)
    
    with open(verbalizer_outputs,'r',encoding='utf-8') as f:
...
if __name__ == "__main__":
    input_data = [
        "<H> [title] <T> Valentin Lavigne <H> member of sports team <T> FC Lorient <H> FC Lorient start time <T> 01 January 2014 <H> FC Lorient end time <T> 01 January 2016 ",
        "<H> [title] <T> Daimon Station ( Aichi ) <H> adjacent station <T> Kitano-Masuzuka Station <H> Kitano-Masuzuka Station towards <T> Kōzōji Station ",
        "<H> [title] <T> Gharbia Governorate <H> population <T> +4999633 "
    ]
    verbalizer(input_data=input_data)