...
from kninjllm.llm_retriever.E5.infer import infer
from kninjllm.llm_retriever.E5.test import test
from kninjllm.llm_retriever.E5.train import train
    ...
    def run(
        ...
        if self.executeType == "infer":
            ...
            if self.logSaver is not None:
                self.logSaver.writeStrToLog("Function -> E5_Retriever -> run | Given the search text, return the search content ")
            ...
            output_path = RootConfig.root_path + "TEMP_E5_test_output_" + test_data_info['dataset_path'].replace("/", "_").replace("\\","_").replace(".","")
            print("--------------output_path---------------")
            print(output_path)
            ...
            os.chdir(RootConfig.root_path + "kninjllm/llm_retriever/E5")
            if train_data_info['dataset_path'] == "":
                return {"final_result": {}} 
            print("train_data_path")
            print(train_data_info['dataset_path'])
            out_dir_name = "TEMP_TRAIN_E5"
