...
from kninjllm.llm_retriever.BERT.model_eval import get_model
...
def infer(
    ...
    for idx, data in enumerate(passage):
        if  "BERT_embedding" in data and data['BERT_embedding'] != None:
            corpus_embedding = data['BERT_embedding']
        else:
            text = [data['content']]
