...
from kninjllm.llm_retriever.BERT.infer import infer
from kninjllm.llm_retriever.BERT.test import test
from kninjllm.llm_retriever.BERT.train import train


@component
class BERT_Retriever:
    ...
    def run(
        ...
        if self.executeType == "infer":
            ...
            if self.logSaver is not None:
                self.logSaver.writeStrToLog("Function -> BERT_Retriever -> run | Given the search text, return the search content ")
            ...
            output_path = RootConfig.root_path + "TEMP_BERT_test_output_" + test_data_info['dataset_path'].replace("/", "_").replace("\\","_").replace(".","")
            print("--------------output_path---------------")
            print(output_path)
            ...
            os.chdir(RootConfig.root_path + "kninjllm/llm_retriever/BERT")
            if train_data_info['dataset_path'] == "":
                return {"final_result": {}} 
            print("train_data_path")
            print(train_data_info['dataset_path'])
            
            out_dir_name = "TEMP_TRAIN_BERT"
