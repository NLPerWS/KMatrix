...
class Args_encode():
    ...
    model_name: str = field(
        default=RootConfig.DPR_model_path,
...
def worker_function(device, eval_data, corpus, shared_hidden_size,model_path,dataset_path,output_path):
    ...
    args_encode.save_dir = output_path
...
def main_eval(model_path,dataset_path,output_path):
    ...
    args_eval.save_dir = output_path
    ...
    print("------------maineval-metrics----------")
    print(metrics)
    
    
    if os.path.exists(copus_embed_save_path):
        os.remove(copus_embed_save_path)
        print(f"{copus_embed_save_path} has been deleted.")
    else:
        print(f"{copus_embed_save_path} does not exist.")
    if os.path.exists(query_embed_save_path):
        os.remove(query_embed_save_path)
        print(f"{query_embed_save_path} has been deleted.")
    else:
        print(f"{query_embed_save_path} does not exist.")
    return metrics
