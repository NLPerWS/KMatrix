...
from kninjllm.llm_retriever.DPR.utils_infer import logger, pool, move_to_cuda, create_batch_dict
...
class DenseEncoder(torch.nn.Module):
    ...
    def encode_corpus(self, corpus, batch_size=32, **kwargs) -> np.ndarray:
        ...
        if type(corpus) is dict:
            sentences = [
                (corpus["title"][i] + " " + corpus["content"][i]).strip()
                if "title" in corpus
                else corpus["content"][i].strip()
            ...
            if type(corpus[0]) is str:
                ...
                sentences = [
                    (doc["title"] + " " + doc["content"]).strip() if "title" in doc else doc["content"].strip()
                    for doc in corpus
                ]
            
        return self.encode(sentences, batch_size, max_length=self.max_passage_length)

    def encode_corpus_plus(self, corpus, batch_size=32, **kwargs) -> np.ndarray:
        """Used for encoding the corpus of retrieval tasks"""
        if type(corpus) is dict:
            sentences = [
                (corpus["title"][i] + " " + corpus["content"][i]).strip()
        ...
        allembeddings = self.encode_plus(sentences, batch_size, max_length=self.max_passage_length)
        allids = [item["id"] for item in corpus]
    ...
    def encode(self, sentences, batch_size=32, **kwargs) -> np.ndarray:
        ...
        for start_idx in tqdm.tqdm(range(0, len(input_texts), batch_size), desc='encoding', mininterval=10):
            ...
            with torch.cuda.amp.autocast():
                ...
                if self.normalize and 'contriever' not in self.args.model_path:
    ...
    @torch.no_grad()
    def encode_plus(self, sentences, batch_size=1, **kwargs) -> np.ndarray:
        """ Returns a list of embeddings for the given sentences.
        Args:
            sentences (`List[str]`): List of sentences to encode
            batch_size (`int`): Batch size for the encoding
        ...
        for start_idx in tqdm.tqdm(range(0, len(input_texts), batch_size), desc='encoding', mininterval=10):
            ...
            with torch.cuda.amp.autocast():
                ...
                if self.normalize and 'contriever' not in self.args.model_path:
                    embeds = F.normalize(embeds, p=2, dim=-1)
                encoded_embeds.append(embeds.squeeze().cpu().half())
