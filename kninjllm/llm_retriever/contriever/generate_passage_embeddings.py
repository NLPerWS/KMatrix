...
def main(args):
    ...
    print("type passges  " , type(passages))
    print(" passges 0 " , passages[0])
...
def main_do_embedding(passages,model_name_or_path=RootConfig.contriever_model_path,no_fp16=False,shard_id=0,num_shards=1):
    ...
    parser.add_argument(
        "--per_gpu_batch_size", type=int, default=1024, help="Batch size for the passage encoder forward pass"
    )
    ...
    print("allids len ",len(allids))
    print("allids[0] ",allids[0])
    
    print("allembeddings len ",len(allembeddings))
    print("allembeddings [0]",allembeddings[0])
